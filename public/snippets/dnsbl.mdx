...
DNSBL fails closed earlier in the chain for **list-based** domain risk; URL and content rules still apply for paths, categories, and payloads.
...
- SafeSquid configured as the primary DNS resolver (see [Supporting Services: BIND](/safesquid_swg/interface/bind)).
...
1. [Access the SafeSquid User Interface](/safesquid_swg/interface/configuration_portal).
...
- **Logs**: In [Security Logs](/use_cases/audit_and_forensics/security_logs), confirm entries for blocked requests (action/result indicating DNSBL block).
...
| **Direct IP access bypasses DNSBL** | Users access sites by IP instead of hostname | **Limitation:** DNSBL only works on DNS queries, not direct IP connections. **Mitigation:** Use [Access Restriction](/use_cases/access_restriction/access_restriction) to block by IP range or category. |
...
| Log evidence via [Security Logs](/use_cases/audit_and_forensics/security_logs) | **Confirmed** | Linked doc |
| Direct-IP bypass limitation | **Confirmed** | This page, [Access Restriction](/use_cases/access_restriction/access_restriction) |

## Next steps

- Return to [DNS Security](/use_cases/dns_security/dns_security).
- Align resolver placement: [BIND](/safesquid_swg/interface/bind).
