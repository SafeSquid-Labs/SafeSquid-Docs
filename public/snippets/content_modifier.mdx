...
**SafeSquid-side:** SafeSquid deployed and operational; admin access to the configuration interface. [SSL Inspection](/use_cases/ssl_inspection/ssl_inspection) enabled if modifying HTTPS response body. [Access Restriction](/use_cases/access_restriction/access_restriction) and profiles configured so modifier policies can target the right connections.



## Enable and configure Content Modifier in SafeSquid

Access the SafeSquid interface via the [Configuration Portal](/safesquid_swg/interface/configuration_portal).
...
### Related sections
- [Header Re-Write](/use_cases/header_rewrite/header_obfuscation) for filtering or modifying specific headers.
- [Cookie Inspection](/use_cases/cookie_inspection/cookie_inspection) for cookie filtering and policy.
- [Data Leakage Prevention](/use_cases/data_leakage_prevention/data_leakage_prevention) for DLP and compliance templates.

## Next steps

Use with [Access Restriction](/use_cases/access_restriction/access_restriction) profiles; enable [SSL Inspection](/use_cases/ssl_inspection/ssl_inspection) when modifying HTTPS response body.
