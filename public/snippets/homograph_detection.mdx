...
Homograph detection works best alongside [DNSBL](/use_cases/dns_security/dnsbl) (block known-bad domains), [SSL Inspection](/use_cases/ssl_inspection/ssl_inspection) (detect fake certificates), and user security awareness training.
:::
...
- DNS resolution handled by SafeSquid (see [Supporting Services: BIND](/safesquid_swg/interface/bind))
...
1. Configure your browser to use SafeSquid as proxy (see [Connect Your Client](/getting_started/client_configuration/connect_your_client))
...
- **Layer defenses:** Combine with [DNSBL](/use_cases/dns_security/dnsbl) to block known-malicious domains
- **Inspect certificates:** Enable [SSL Inspection](/use_cases/ssl_inspection/ssl_inspection) to detect fake TLS certificates on lookalike domains
- **Monitor attempts:** Use [Security Logs](/use_cases/audit_and_forensics/security_logs) and [Reporting](/use_cases/audit_and_forensics/reporting_module) to track homograph attack patterns
