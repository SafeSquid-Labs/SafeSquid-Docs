...
<Note>
Open the Web UI at `http://safesquid.cfg/` from a machine that can reach the appliance. Policy lives in the Web UI; process tunables live in [startup.ini](/configuration/start_here/startup_ini).
</Note>
...
If it is not running: `/etc/init.d/safesquid start`. Details: [Daemon, service, and files](/configuration/start_here/daemon).
...
- If Listen is empty, SafeSquid falls back to `LISTEN_IP` / `LISTEN_PORT` in [startup.ini](/configuration/start_here/startup_ini).
{/* source: https://www.safesquid.com/md/first-configuration.md */}
...
More detail: [Network settings](/configuration/application_setup/network_settings_and_listeners).
...
2. Leave **Kerberos / SSO** off for this first pass (add it later; see [Authentication](/configuration/start_here/authentication)).
...
Full guide: [Access restrictions](/configuration/application_setup/access_restrictions).
...
**Expected result:** matching requests show a block page; Detailed logs show a profiles-related `filter_name` / reason. See [Access Profiles](/configuration/restriction_policies/access_profiles).
</Tip>
...
3. On a test network only, set **System configuration → Send Debugging Headers To → CLIENT** and inspect response headers in the browser. See [Debug response headers](/configuration/start_here/debug_response_headers).
4. Turn debugging headers back to **NONE** when finished.

Full workflow: [Logging and troubleshooting](/configuration/start_here/logging).
...
- [Authentication](/configuration/start_here/authentication) — Kerberos / SSO or System authentication so logs show real usernames
- [Integrate LDAP](/configuration/application_setup/integrate_ldap) — groups for policy
- [HTTPS Inspection](/configuration/real_time_content_security/https_inspection) — decrypt HTTPS when your policy requires it (plan client trust carefully)
- [SSqore](/configuration/application_setup/ssqore) and [Categorize Web-Sites](/configuration/custom_settings/categorize_web_sites) — category-based Access Profiles
- [Application Signatures](/configuration/custom_settings/application_signatures) — tag client applications for policy
- [SqScan](/configuration/real_time_content_security/sqscan) or [Clam antivirus](/configuration/real_time_content_security/clam_antivirus) — malware scanning
- [Subscription](/configuration/infrastructure_and_access/subscription) — license and feed updates
- [Architecture](/configuration/start_here/architecture) — how sections fit together
