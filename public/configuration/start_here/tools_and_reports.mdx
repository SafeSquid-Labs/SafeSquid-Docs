...
- **password** — encrypt passwords for policy fields (for example HTTPS Inspection cached cert password). Fields that accept an encrypted password value show a dedicated button in the Web UI that opens this tool, rather than expecting you to type or paste an already-encrypted value by hand.
- **Upload templates** — upload custom block/info templates used by DENY actions. Any policy section's Template field (Access Profiles DENY actions, ClamAV, DNS Blacklist, and others) can reference an uploaded template by name.
