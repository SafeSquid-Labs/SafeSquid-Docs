...
| Unsure which profile matched | Detailed logs + [debug headers](/configuration/start_here/debug_response_headers) on a test client |
...
1. Confirm proxy host/port and [Listen](/configuration/application_setup/network_settings_and_listeners).
...
3. If rows show access deny, fix [Access restrictions](/configuration/application_setup/access_restrictions) (subnet, Interface, Default Access Policy).
</Tip>
...
3. Check profiles and categories on that row; adjust the matching [Access Profiles](/configuration/restriction_policies/access_profiles) entry or enable Trace Entry on the suspect rule.
</Tip>
...
1. Note whether Kerberos / SSO or System authentication is enabled ([Authentication](/configuration/start_here/authentication)).
2. Watch Native logs for `security:`, `pam:`, `ldap:`. The lines around the failed attempt usually name the specific step that failed — the password cache, the local system check, or the directory bind.
...
For live policy visibility on a single request, enable **System configuration → Send Debugging Headers To → CLIENT** (test networks only). Browser tools show `X-SafeSquid-Profiles`, categories, user, and module headers. See [Debug response headers](/configuration/start_here/debug_response_headers).
...
1. Reproduce the issue once and confirm a corresponding entry appears in Detailed logs with the expected timestamp, client, and URL.
2. Confirm `filter_name` and `filtering_reason` on that entry name the module actually under suspicion before changing that module's configuration.
3. If a Native log family or Trace Entry was enabled for the investigation, confirm the expected line appears, then turn it back off.
4. After a fix, reproduce again and confirm the new entry shows the expected outcome.
