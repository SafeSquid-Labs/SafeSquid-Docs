...
<Tabs>
  <Tab title="Global">
    ...
    - **Enabled (`header_enabled`)** — turns Header filter on or off for all connections, unless the matching Access entry's **Bypass** grants **Header filtering**.
    - **Policy (`header_policy`)** — the default outcome when no Allow/Deny row matches a header field: **Deny** strips it, **Allow** forwards it. It also decides which list is walked first (see step 2–3 above).
  ...
  <Tab title="View headers">
    ...
    A read-only panel listing the header fields SafeSquid has observed, provided so you can build
    Allow, Deny and Insert rows against names that actually appear on your traffic instead of
    guessing them.

    {/* NEEDS-SME-REVIEW: tab confirmed present in the console menu (build 2026.0627.1344.3, 2026-09-07) but its contents and any controls were not exercised. Confirm what the panel shows and whether it is filterable before expanding this. */}

    **Missing:** the panel's exact columns and whether it can be filtered or exported are not
    documented here — they were not verified against a live build. Read it directly in the
    console, and escalate to the SME before relying on it as an audit record.
