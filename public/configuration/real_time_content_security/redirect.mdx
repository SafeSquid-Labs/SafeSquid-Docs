...
```mermaid
flowchart TB
req[Client request or response Location header] --> walk[Walk enabled rows top to bottom]
walk --> match{Profiles and direction and URL regex all match?}
match -->|No| next[Next row]
match -->|Yes| mode{302 redirect field}
mode -->|Yes| send302[Send HTTP 302 to client with rewritten URL]
mode -->|No| fetch[Rewrite proto host file and port then fetch internally]
```

## Overview

The `Redirect` section allows SafeSquid to intercept requests for specific URLs and either internally rewrite them (fetching content from a new destination on behalf of the client) or send an HTTP 302 redirect back to the client’s browser. Typical uses include enforcing a search engine's safe-search mode, sending users away from a decommissioned internal site, or rewriting an insecure link a server returned into its secure equivalent before it reaches the browser.
...
- **Enabled (enabled)**: Toggles URL redirection globally, on for all connections unless the matching Access restrictions entry's Bypass grants Redirect.
...
<Frame caption="Redirect — Redirection policies rows">
  <img src="/images/configuration/redirect-redirection_policies.webp" alt="SafeSquid console showing Redirection policies rows enforcing search-engine safe search, the Edit Policies icon on the first row circled" />
</Frame>
