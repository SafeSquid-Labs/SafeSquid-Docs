...
```mermaid
flowchart TB
start[Request host] --> uncategorized{Website categories empty?}
uncategorized -->|Yes| heuristics[Run uncategorized heuristics for name length then homograph then unique host count]
uncategorized -->|No| category
heuristics --> hBlock{Heuristic failed?}
hBlock -->|Yes| block[DO NOT BYPASS and HTTP 451 block page]
hBlock -->|No| category
category[Walk Threatful Categories rows] --> catMatch{Category match?}
catMatch -->|Yes| block
catMatch -->|No| request[Walk Threatful Requests rows]
request --> reqMatch{Request type match?}
reqMatch -->|Yes| block
reqMatch -->|No| dnsbl[Append DNSBL Domain suffix and resolve A record]
dnsbl --> dnsblMatch{A record in Blocked IP addresses?}
dnsblMatch -->|Yes| block
dnsblMatch -->|No| ipcheck[Compare A record and reverse DNSBL answer and GeoIP country]
ipcheck --> ipMatch{Match Blocked IP addresses or Threatful Countries?}
ipMatch -->|Yes| block
ipMatch -->|No| allow[Allow]
```
...
{/* source: https://www.safesquid.com/md/dnsbl.md */}
Category-based blocking alone always lags a newly-registered attack domain by however long it takes the categorisation feed to see and label it. The uncategorized-domain heuristics in this section — unusually long hostnames, an unusual number of distinct hosts under one uncategorized domain, and homograph (lookalike-character) detection — exist specifically to close that gap, catching suspicious new domains on structural grounds before any external feed has categorized them at all.

{/* source: https://www.safesquid.com/md/dnsbl.md */}
This section blocks on:

- Threat categories — Malware, Phishing & Fraud, Botnets, Compromised, Hacking, Illegal Software, Anonymizers, Spam Sites.
- The same content/compliance categories Access Profiles' Categories field uses — for example Nudity, Gambling.
- Country of the resolved server.
- Classic external blocklist (DNSBL) lookups.
...
{/* source: https://www.safesquid.com/md/dnsbl.md */}
Category and request-type matches are tried first, before any DNS lookup, because they are the cheapest checks in the sequence.
...
Open **Configure → Real time content security → DNS Blacklist → DNS and IP Black Listing
Policies**. Row fields are Enabled, Comment, and Threatful Categories (or Threatful Requests) —
the top row in the capture below is hidden since its category list names a CSAM category not
suitable to publish verbatim; the remaining rows show the field shape clearly.

<Frame caption="DNS Blacklist — DNS and IP Black Listing Policies rows">
  <img src="/images/configuration/dns_blacklist-policies.webp" alt="SafeSquid console showing DNS Blacklist policy rows with Threatful Categories including Malware and Phishing, the Edit Policies icon circled" />
</Frame>
...
{/* source: https://www.safesquid.com/md/dnsbl.md */}
5. If a category or country block was expected but did not fire, confirm the destination's own categorisation or resolved address first — this section can only act on categories and addresses already available to it.
