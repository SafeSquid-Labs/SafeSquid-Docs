...
<Frame caption="SqScan policy flow">
  <img src="/images/configuration/sqscan_flowchart.svg" alt="SqScan policy flow" />
</Frame>
...
The eight Malware Types checkboxes — **VIRUS**, **SPYWARE**, **ADWARE**, **SPAM**, **ROOTKIT**, **DIALER**, **MALICIOUS_APP**, and **ARCHBOMB** — set bit flags on the policy but are not passed to the scanner API. Detection uses the engine’s built-in signature set.
...
<Frame caption="SqScan — Virus scanning policies row">
  <img src="/images/configuration/sqscan-virus_scanning_policies.webp" alt="SafeSquid console showing a SqScan Virus scanning policies row with STANDARD Malware Security Level, the Edit Policies icon circled" />
</Frame>
