...
```mermaid
flowchart TB
walk[Walk enabled rows top to bottom] --> match{Profiles and Applies to and session mode match?}
match -->|No| next[Next row]
match -->|Yes| exec[Pipe or File body to Executable]
exec --> parse{stdout parses as well-formed HTTP header?}
parse -->|No| next
parse -->|Yes| extract[Extract X-username and X-profiles on any exit code]
extract --> exit{Exit code}
exit -->|0| replace[Replace request or response content and stop]
exit -->|Non-zero| preserve[Preserve original content]
```
...
{/* source: https://www.safesquid.com/md/external.md */}
This is a genuine extension point into content not otherwise controlled from the Web UI, so treat every referenced executable as trusted code running with SafeSquid's own privileges — it sees live request and response traffic and, depending on how it exits, can replace that traffic outright or inject a username and additional Access Profiles into the connection.
...
{/* source: https://www.safesquid.com/md/external.md */}
<Note>
**`X-username`/`X-profiles` extraction is not conditional on a non-zero exit.** SafeSquid scans the program's output for those two headers and acts on them whenever the output parses as a well-formed HTTP header, on any exit code, including 0. `X-profiles` values already present on the connection are not duplicated.
</Note>
...
Open **Configure → Custom Settings → External applications**. **Global** holds the master Enabled
switch; **Application policies** holds the rule rows (Executable, Type, Applies to, Run once per
session, Send header) once at least one row is added.

<Frame caption="External applications — Global tab">
  <img src="/images/configuration/external_applications-global.webp" alt="SafeSquid console showing the External applications Global tab with the Enabled toggle set to FALSE, the Edit Policy icon circled" />
</Frame>
...
{/* source: https://www.safesquid.com/md/external.md */}
5. Confirm whether unexpected profile or username changes are coming from an entry you assumed only ran on failure — extraction is not exit-code-conditional, so it can happen on exit 0 too.
6. If a REQUESTS entry is meant to answer the client directly, confirm its script returns a complete HTTP *response* header (not a request header) on exit code 0 — the wrong header type parses differently and won't produce the intended short-circuit.
