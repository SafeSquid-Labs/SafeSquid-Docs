...
**Suggested Profiles** shows built-in profile ideas you can pick when configuring cookie-related policy. It is a **UI-only** naming aid — no C++ section loads this configuration at runtime.

{/* source: https://www.safesquid.com/md/buitins.md */}
{/* NEEDS-SME-REVIEW: existing intro and Entry fields scope this section to cookie profile tags only; upstream https://www.safesquid.com/md/buitins.md documents fourteen catalogs feeding many sections. Scope statement needs an SME decision before the narrow wording is corrected. */}
<Note>
...
<Note>
To enforce cookie behaviour, configure [Cookie filter](/configuration/restriction_policies/privacy_control) and reference the same tag names in [Access Profiles](/configuration/restriction_policies/access_profiles) where `cookie_profiles` is used.
</Note>
...
- [Cookie filter](/configuration/restriction_policies/privacy_control)
