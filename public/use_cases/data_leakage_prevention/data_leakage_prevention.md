...
- **Image Analyser** inspects image-based content for visual threat intelligence and sensitive-information egress
...
→ [Configure Text Analyser](/Text_Analyser)

### 2. Image Analyzer Detects Visual Content

Inspects images in uploads and attachments:
- **Image-based sensitive-content analysis:** Detects risky or sensitive visual content in uploaded images
- **Explicit content detection:** Blocks NSFW uploads to corporate Slack, SharePoint

**Use case:** Employee screenshots customer data from internal CRM and uploads the image to personal cloud storage. Image-based analysis helps prevent that visual egress path from becoming a blind spot.

→ [Configure Image Analyser](/Image_Analyser_AI)
...
→ [Load Compliance Templates](/Compliance_Templates)
...
**Action:** [Enable SSL Inspection](/SSL_Inspection) and deploy the Root CA to client devices.
...
**Action:** [Load Compliance Templates](/Compliance_Templates) in the Text Analyser configuration.
...
**Action:** [Configure Text Analyser](/Text_Analyser)
...
**Action:** [Configure Access Restrictions](/Access_Restriction) with DLP profile matching.
...
**Action:** [Configure SIEM Integration](/Integrations) with syslog forwarding.
...
SafeSquid's broader evidence model also includes bypass, privileged-access, and deep-content security records that can help explain why a DLP decision did or did not occur.
...
**Solution:** [Enable SSL Inspection](/SSL_Inspection) first, deploy Root CA to clients, then enable DLP.
...
**Solution:** Enable [archive decompression](/Content_Modifier) in Content Modifier. SafeSquid will extract and scan ZIP/RAR contents before allowing upload.
...
**What breaks:** User screenshots customer data from CRM, uploads image to personal cloud. Text-only controls miss the image path.  
**Solution:** Enable [Image Analyser (AI-powered)](/Image_Analyser_AI) so image-based egress is not a blind spot.
...
**Prerequisites:**
- [SSL Inspection](/SSL_Inspection) — Required to inspect encrypted uploads (90% of traffic)
- [Authentication](/Authentication) — User identity needed for per-user DLP policies

**Next Steps:**
- [Text Analyser Configuration](/Text_Analyser) — Fine-tune regex patterns
- [Image Analyser (AI-powered)](/Image_Analyser_AI) — OCR for screenshots
- [Access Restrictions](/Access_Restriction) — Link DLP detections to block/log actions
- [SIEM Integration](/Integrations) — Forward DLP alerts to Splunk, QRadar, etc.

**Troubleshooting:**
- [DLP False Positives](/Troubleshooting) — How to tune regex patterns, whitelist test data

**Use Cases:**
- [Block Personal Gmail, Allow Corporate](/Block_Personal_Gmail_Allow_Google_Corporate_Accounts) — Prevent data leaks via personal email
- [Block Keywords in Uploads](/Block_Emails_or_Files_including_archives_or_Social_Posts_using_Keywords) — Detect "confidential" in social posts
