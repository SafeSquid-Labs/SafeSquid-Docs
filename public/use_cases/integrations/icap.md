...
keywords:
  - SafeSquid ICAP integration
  - ICAP antivirus scanning
  - content scanning offload
  - third-party malware detection
  - ICAP server configuration
  - proxy ICAP connectivity
---
...
ICAP concentrates on leveraging edge-based devices (proxies and caches) to help deliver value-added services. At the core of this process is a cache that will proxy all client transactions and will process them through ICAP Web servers.
These ICAP servers are focused on a specific function, for example, add insertion, virus scanning, content translation, language translation, or content filtering.
...
## [Access the SafeSquid interface](/safesquid_swg/interface/configuration_portal)
...
### Threat ID Header
ICAP server's response header that contains the loggable identification of the Detected Threat.
...
Enable or Disable above policies as per the requirement or add new policy for new
requirement.
...
- **Interface Checks**: In [Configuration Portal](/safesquid_swg/interface/configuration_portal), Real-time content security → ICAP: Global enabled, policy entries show correct ICAP server Host, Port, and File; Applies to (Requests/Responses) matches intent.
