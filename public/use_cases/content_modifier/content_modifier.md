...
**SafeSquid-side:** SafeSquid deployed and operational; admin access to the configuration interface. [SSL Inspection](/SSL_Inspection) enabled if modifying HTTPS response body. [Access Restriction](/Access_Restriction) and profiles configured so modifier policies can target the right connections.



## Enable and configure Content Modifier in SafeSquid

Access the SafeSquid interface via the [Configuration Portal](/Configuration_Portal).
...
### [Rewriting Policies Reference](/Rewriting_Policies_Reference)
...
### Related sections
- [Header Re-Write](/Header_Obfuscation) for filtering or modifying specific headers.
- [Cookie Inspection](/Cookie_Inspection) for cookie filtering and policy.
- [Data Leakage Prevention](/Data_Leakage_Prevention) for DLP and compliance templates.

## Next steps

Use with [Access Restriction](/Access_Restriction) profiles; enable [SSL Inspection](/SSL_Inspection) when modifying HTTPS response body.
