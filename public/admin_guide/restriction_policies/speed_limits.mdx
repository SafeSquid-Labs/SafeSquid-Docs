...
- **Action (action)**: `Allow` or `Deny`. Whether this field has any runtime effect is disputed — see the warning below.
...
<Warning>
**Action's runtime effect is contradicted by the product's own sources.** The console's field help states Action is enforced: set `Deny` and matching requests are blocked outright, set `Allow` and limits apply until a cap is reached, at which point the request is denied automatically. Source-level analysis of the limits code found the opposite — that Action is saved in configuration but never read, so limits apply on numeric caps regardless of what Action is set to.

Neither claim is written here as fact. Determine it on your own system before relying on Action: create a test row scoped to a disposable profile tag, set Action to `Deny` with all numeric caps left blank, and send a request carrying that tag. If the request is blocked, Action is enforced on your build. If it succeeds, Action is inert and only the numeric caps matter.
</Warning>
...
- **Action** — Disputed. Console help describes it as enforced; source analysis found it unread. See the warning above and test on your own build.
...
<Frame caption="Speed Limits — Set limits rows">
  <img src="/images/admin_guide/speed_limits-set_limits.webp" alt="SafeSquid console showing Speed Limits rows with Download rate and transfer limit fields, the Edit Policies icon on the first row circled" />
</Frame>
