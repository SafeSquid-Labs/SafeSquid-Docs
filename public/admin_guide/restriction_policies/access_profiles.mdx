...
1. [Time Profiler](/admin_guide/custom_settings/time_profiler) → `time_schedules`
2. [Request Types](/admin_guide/custom_settings/request_types) → `request_types`
3. Domain categorization → `website_categories`
4. **Access Profiles** → `profiles` + `action`
5. [Response Types](/admin_guide/custom_settings/response_types) on response headers/body → updates `response_types`; Access Profiles runs again
...
- **Time Schedule** — Tags from [Time Profiler](/admin_guide/custom_settings/time_profiler).
- **Request Types** — Tags from [Request Types](/admin_guide/custom_settings/request_types).
- **Response Types** — Tags from [Response Types](/admin_guide/custom_settings/response_types); requires response header when field set.
...
<Frame caption="Access Profiles — Default Policies list">
  <img src="/images/admin_guide/access_profiles-default_policies.webp" alt="SafeSquid console showing the Access Profiles Default Policies list, with the Edit Policies pencil icon on the first row circled" />
</Frame>
