...
Source: `sources/modules/sqscan/sqscan.cpp` — `SqScanSection::find()`, scan level handling.
...
Malware Security Level **BYPASS** skips scanning (`SQSCAN_BYPASS`). Values STANDARD, HIGH, and PARANOID are stored but the engine uses fixed scan options — only **BYPASS** changes runtime behaviour today.
...
Virus detection (`QH_API_VIRUS_FOUND`) sets `POLICY_DO_NOT_BYPASS`. Access restrictions **Bypass** with antivirus skips SqScan entirely.
...
- **Malware Security Level (scan\_flag)** — BYPASS skips SqScan. Other levels label the row but do not alter scan depth in current code.
