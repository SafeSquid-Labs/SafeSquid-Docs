...
<Note>
Configure authentication mechanisms in [Authentication](/admin_guide/infrastructure/authentication). LDAP directory setup is in [Integrate LDAP](/admin_guide/infrastructure/integrate_ldap). Content allow/deny is in [Access Profiles](/admin_guide/policymgr/access_profiles).
</Note>
...
UI label **Kerberos / SSO**; internal field `ntlm_authentication`. When TRUE, SafeSquid offers **Negotiate** (GSSAPI/Kerberos). When FALSE, Negotiate is not advertised. Requires `HTTP.keytab` for SSO to succeed. See [Authentication](/admin_guide/infrastructure/authentication).
