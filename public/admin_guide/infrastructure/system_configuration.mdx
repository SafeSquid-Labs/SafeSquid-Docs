...
Each getter walks **Compression and buffering policies** top-down. First enabled row whose `profiles` pass `profile_find()` wins. Empty profiles match all. No match → built-in defaults.

### CONNECT ports (cportrange)

`cportrange_check()` applies only when a row matched. Port must be in the row list or CONNECT is blocked. When no row matched, all CONNECT ports are allowed (unless blocked elsewhere).
...
- **Send Debugging Headers To** — CLIENT, SERVER, BOTH, or NONE — see [Debug headers](/admin_guide/reporting/debug_response_headers).
