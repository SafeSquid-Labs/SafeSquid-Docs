...
| Unsure which profile matched | Detailed logs + [debug headers](/admin_guide/reporting/debug_response_headers) on a test client |

## Log families

- **Native logs** — Internal diagnostics from `putlog()`. Each line is timestamped and tagged (for example `security:`, `ldap:`, `cache:`). File: `/var/log/safesquid/native/safesquid.log`
...
1. Confirm proxy host/port and [Listen](/admin_guide/infrastructure/network_settings).
...
3. If rows show access deny, fix [Access restrictions](/admin_guide/infrastructure/access_restrictions) (subnet, Interface, Default Access Policy).
</Tip>
...
3. Check profiles and categories on that row; adjust the matching [Access Profiles](/admin_guide/policymgr/access_profiles) entry or enable Trace Entry on the suspect rule.
</Tip>

<Tip>
**3 — “Login keeps asking for a password”**

1. Note whether Kerberos / SSO or System authentication is enabled ([Authentication](/admin_guide/infrastructure/authentication)).
...
For live policy visibility on a single request, enable **System configuration → Send Debugging Headers To → CLIENT** (test networks only). Browser tools show `X-SafeSquid-Profiles`, categories, user, and module headers. See [Debug response headers](/admin_guide/reporting/debug_response_headers).
