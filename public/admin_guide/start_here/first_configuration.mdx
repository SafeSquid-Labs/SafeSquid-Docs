...
<Note>
Open the Web UI at `http://safesquid.cfg/` from a machine that can reach the appliance. Policy lives in the Web UI; process tunables live in [startup.ini](/admin_guide/system_operations/startup_parameters).
</Note>
...
- If Listen is empty, SafeSquid falls back to `LISTEN_IP` / `LISTEN_PORT` in [startup.ini](/admin_guide/system_operations/startup_parameters).
...
More detail: [Network settings](/admin_guide/network_and_access/network_settings_and_listeners).
...
Full guide: [Access restrictions](/admin_guide/network_and_access/access_restrictions).
...
- [HTTPS Inspection](/admin_guide/https_inspection/https_inspection) — decrypt HTTPS when your policy requires it (plan client trust carefully)
- [SSqore](/admin_guide/infrastructure_and_access/ssqore) and [Categorize Web-Sites](/admin_guide/policies_and_profiles/web_categories) — category-based Access Profiles
- [Application Signatures](/admin_guide/policies_and_profiles/application_signatures) — tag client applications for policy
- [SqScan](/admin_guide/malware_and_data_protection/sqscan) or [Clam antivirus](/admin_guide/malware_and_data_protection/clamav) — malware scanning
