...
- **[Categorize Web-Sites](/admin_guide/policies_and_profiles/web_categories)** — Local category overrides you maintain. Takes precedence over cloud results for matching hosts on the hot path.
...
- **DNS\_CAT\_ZONE** — In [startup.ini](/admin_guide/system_operations/startup_parameters) (default `.c.ssquid.in`). Set to `.` to disable DNS categorisation zone use.
- **UPDATE\_INTVL / UPDATE\_RETRY\_DELAY** — How often the appliance fetches signature and feed updates ([startup.ini](/admin_guide/system_operations/startup_parameters)).
...
1. Use [Categorize Web-Sites](/admin_guide/policies_and_profiles/web_categories) to assign `Business-Partners` to `partner.example.com`.
...
- [Categorize Web-Sites](/admin_guide/policies_and_profiles/web_categories)
...
- [startup.ini](/admin_guide/system_operations/startup_parameters)
