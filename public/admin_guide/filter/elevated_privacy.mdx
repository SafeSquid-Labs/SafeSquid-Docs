...
Source: `sources/modules/elevated/elevated.cpp` — `elevatedSection::match()`, request/response header processors.

### First match wins

On each request and response header pass, enabled Elevated policy rows are walked top to bottom. The **first** matching row sets the privacy level for that connection (`break` after first match).
