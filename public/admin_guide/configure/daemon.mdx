...
- **Startup settings** — `/opt/safesquid/startup.ini`, `/opt/safesquid/setup.ini` — see [startup.ini tunables](/admin_guide/configure/startup_ini) (not in sectionMaster)
...
Background UPDATE hooks refresh application signatures, content signatures, categorisation feeds, and related databases. License state is managed under [Subscription](/admin_guide/infrastructure/subscription) in the Web UI; see [Cloud / categorisation feeds](/admin_guide/infrastructure/cloud_feeds) for paths under `/var/lib/safesquid/`.
