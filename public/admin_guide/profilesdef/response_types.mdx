...
The **Response Types** section (`safesquid-respProfiles(5)`) labels HTTP responses for [Access Profiles](/admin_guide/policymgr/access_profiles) and content filters. It does not block by itself.

## Core mechanics

Validated against `sources/src/response_profiles.cpp` — `response_profiles_update()`, response hook.

### Two-pass evaluation

1. On response headers — `do_response_profile()` with no body buffer.
