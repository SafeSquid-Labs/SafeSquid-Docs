...
The **Application Signatures** section (`safesquid-applicationSignatures(5)`) tags each HTTP request with application signature strings (for example Webmail, BitTorrent, Chrome). Other sections match those tags with exact string compare — especially [Access Profiles](/admin_guide/policymgr/access_profiles) (Request Types field) and [Request Types](/admin_guide/profilesdef/request_types).

<Note>
Vendor signature databases download on the appliance update schedule. See [startup.ini](/admin_guide/configure/startup_ini) UPDATE settings, [Cloud / categorisation feeds](/admin_guide/infrastructure/cloud_feeds), and [Subscription](/admin_guide/infrastructure/subscription) (expired subscription skips all signature processing).
</Note>

## Core mechanics

Validated against `sources/src/application_signatures.cpp` — `do_application_signatures()`, `application_signatures_update()`.
...
4. Debug header `X-SafeSquid-Application-Signatures` when Send Debugging Headers To includes CLIENT — see [Debug response headers](/admin_guide/reporting/debug_response_headers).
