...
The **Request Types** section (`safesquid-reqProfiles(5)`) labels HTTP requests with tags consumed by [Access Profiles](/admin_guide/policymgr/access_profiles) and other filters. It does not block traffic.

## Core mechanics

Validated against `sources/src/request_profiles.cpp` — `do_request_profile()`, `match_host_list()`, `req_profiles_update()`.
...
<Warning>
**urlcommand field:** parsed in config but not evaluated in `req_profiles_update()`. Use **Protocol** or **File** instead.
</Warning>
