...
The **Content Signatures** section (`safesquid-contentSignatures(5)`) maintains the content-signature database and libmagic MIME detection used across SafeSquid. It helps identify actual file types, bypassing deceptive file extensions — but live connection tagging for policy uses [Response Types](/admin_guide/profilesdef/response_types) in current code.

<Note>
Custom rows are merged into `content4.xml` under `/var/lib/safesquid/content_signatures/`. Tag names still appear in DLP and UI pickers even when request-time hooks are stubbed.
</Note>

## Core mechanics

Validated against `sources/src/content_signatures.cpp` — `content_signatures_update()`, `content_signatures_update_post()`, `update_content_signatures()`.
...
4. `content_signatures_update()` and `content_signatures_update_post()` are **empty stubs** — list rows do not add or remove connection tags at request time in current code.

<Warning>
For live response labelling by MIME or extension, use [Response Types](/admin_guide/profilesdef/response_types). Content Signatures rows still matter for database maintenance, autocomplete in other sections (for example DLP), and libmagic setup.
</Warning>

### Global Enabled

When off, `Content_Signatures_Section::is_enabled()` returns FALSE — signature database download/reload and MIME library setup are affected. List rows are still persisted even when hooks are stubbed.
...
- **Content File Extensions** — Extension regex rows plus extension tags. `content_signatures_update_post()` is an empty stub.
