...
- **ICP peer type / ICP port** — When global CARP is off: **NONE** gives fixed priority 1; **PARENT** forwards on ICP miss; **SIBLING** forwards only on ICP HIT. UDP ICP uses the ICP port on this row; global ICP listen port and timeout come from [Caching](/admin_guide/application_setup/accelerators).
{/* source: https://www.safesquid.com/md/forward.md */}
...
<Frame caption="Proxy chain — Forwarding proxies row">
  <img src="/images/admin_guide/proxy_chain-forwarding_proxies.webp" alt="SafeSquid console showing a Forwarding proxies row with an upstream Proxy and Port, the Edit Policies icon circled" />
</Frame>
