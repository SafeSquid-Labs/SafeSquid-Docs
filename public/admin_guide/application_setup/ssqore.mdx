...
**SSqore** categorizes websites by the likely nature of their content. It queries SafeSquid's Content Categorisation Service (CCS) and adds category names to the connection so [Access Profiles](/admin_guide/restriction_policies/access_profiles) and filters can match on them.
...
<Frame caption="SSqore — Global settings">
  <img src="/images/admin_guide/ssqore-global.webp" alt="SafeSquid console showing SSqore Global settings with Enabled set to TRUE, the Edit Policy icon circled" />
</Frame>
...
SSqore is the last-resort cloud tier: it runs only if the site is still uncategorized after local [Categorize Web-Sites](/admin_guide/custom_settings/categorize_web_sites) entries and DNS category feeds have already been checked.
...
Custom or local categories may also come from the [Categorize Web-Sites](/admin_guide/custom_settings/categorize_web_sites) tool and DNS category zone settings in startup.ini; SSqore is the cloud lookup path.
