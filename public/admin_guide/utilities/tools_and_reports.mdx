...
- **Native logs, Detailed logs, Config logs** — see [Logging and troubleshooting](/admin_guide/reporting/logging).
- **Active Connections, Connection Pool, DNS Cache, Password Cache, SSL Certs/Cache** — live state alongside historical logs.
- **License/Users Info, Modules Status** — subscription validity and signature/feed load status ([Subscription](/admin_guide/infrastructure/subscription), [feeds](/admin_guide/infrastructure/cloud_feeds)).
- **LDAP Entries** — directory browse after [Integrate LDAP](/admin_guide/infrastructure/integrate_ldap) is configured.
...
- **[Subscription](/admin_guide/infrastructure/subscription)** — activation key upload and license view.
- **[Categorize Web-Sites](/admin_guide/profilesdef/categorize_web_sites)** — local category overrides (Web UI app, not a policy section list).

## CLI and files

- Init script: `/etc/init.d/safesquid {start|stop|restart|status|foreground|space_clean}` — see [Daemon](/admin_guide/configure/daemon).
