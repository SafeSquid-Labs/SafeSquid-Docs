...
Source: `sources/modules/rewrite/rewrite.cpp` — `RewriteSection::check_match()`, `rewrite_do()`.

### Hook points

- **Client header** (`REWRITE_CLIENT`) — outgoing request headers via `rewrite_reqheader` hook.
- **Server header** (`REWRITE_SERVER`) — response headers via `rewrite_respheader` hook.
- **Body** (`REWRITE_BODY`) — response body via `PROCESS_BUFFCHECK`; MIME regex tested against `Content-Type`.
- **Post body** (`REWRITE_POST`) — upload body modification.
...
For `REWRITE_BODY`, non-empty Mime type regex must match `connection->rheader->content_type`. Empty mime matches any body when other gates pass.
