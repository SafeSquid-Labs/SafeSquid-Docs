...
Source: `sources/src/keywords.cpp` — `keyword_buffcheck()`, cumulative scoring, early exit at threshold.
...
- **Enforcement**: Once the threshold is breached, the connection action becomes `POLICY_DO_NOT_BYPASS`, serving the specified block template and populating the `_SCORE_` and `_REASON_` template variables.
...
1. During response processing, if any enabled row matches the response `Content-Type`, SafeSquid buffers the body (`keyword_buffcheck`).
...
6. At or above Threshold: HTTP 451 block page, bypass disallowed (`POLICY_DO_NOT_BYPASS`), cache code `TCP_DENIED`.
