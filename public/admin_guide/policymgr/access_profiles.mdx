...
1. [Time Profiler](/admin_guide/profilesdef/time_profiler) → `time_schedules`
2. [Request Types](/admin_guide/profilesdef/request_types) → `request_types`
3. Domain categorization → `website_categories`
4. **Access Profiles** → `profiles` + `action`
5. [Response Types](/admin_guide/profilesdef/response_types) on response headers/body → updates `response_types`; Access Profiles runs again
...
- **Time Schedule** — Tags from [Time Profiler](/admin_guide/profilesdef/time_profiler).
- **Request Types** — Tags from [Request Types](/admin_guide/profilesdef/request_types).
- **Response Types** — Tags from [Response Types](/admin_guide/profilesdef/response_types); requires response header when field set.
