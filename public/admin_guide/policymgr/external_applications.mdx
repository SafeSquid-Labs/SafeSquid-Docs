...
Source: `sources/src/external.cpp` — exit code 0 replaces content; non-zero preserves + metadata.
...
6. Run once per session **Yes**: row runs only on external auth pass; non-zero exit sets `SESSION_EXTERNALONCE` to avoid re-auth loops.
