...
- **Upstream Validation**: SafeSquid builds an OpenSSL `X509_STORE` and uses `X509_VERIFY_PARAM` to validate the upstream origin server’s certificate against known CA authorities.
...
- **Encrypted Password (enc\_pwd)**: Encrypted passphrase for the RCSC private key, stored per Setup row. Runtime RCSC load uses the global activation password, not this field, in the current codebase.
...
- **Acceptable Errors in SSL Verification** — Maximum OpenSSL verification error tolerated on the upstream chain. Level 0 (`X509_V_OK`) is strictest. Hostname/IP mismatch is gated separately by Block domain mismatch.
