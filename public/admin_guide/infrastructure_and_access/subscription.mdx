...
{/* NEEDS-SME-REVIEW: this session searched Reports, Configure, and Support (live DOM and nav) for
a standalone "Subscription" page or an activation-key upload control and found neither — the
License Details panel on Support is read-only apart from Refresh. This page's premise of a
separate Subscription section with an upload step could not be verified live. The Upload Config
card on Support was not opened to check (it is on the never-click list for destructive/unverified
controls), so it remains a possible but unconfirmed candidate. No screenshot was captured for this
page as a result — confirm where activation-key upload actually lives before adding one. */}
