...
1. [Time Profiler](/admin_guide/policies_and_profiles/time_profiles) → `time_schedules`
2. [Request Types](/admin_guide/policies_and_profiles/request_profiles) → `request_types`
3. Domain categorization → `website_categories`
4. **Access Profiles** → `profiles` + `action`
5. [Response Types](/admin_guide/policies_and_profiles/response_profiles) on response headers/body → updates `response_types`; Access Profiles runs again
...
- **Time Schedule** — Tags from [Time Profiler](/admin_guide/policies_and_profiles/time_profiles).
- **Request Types** — Tags from [Request Types](/admin_guide/policies_and_profiles/request_profiles).
- **Response Types** — Tags from [Response Types](/admin_guide/policies_and_profiles/response_profiles); requires response header when field set.
