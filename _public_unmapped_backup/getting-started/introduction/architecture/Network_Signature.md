...
1. **Access SafeSquid Configuration**  
   Open the [Configuration Portal](/Configuration_Portal) and click **Configure**.
...
4. **Upgrade to user auth:** For dynamic endpoints (laptops, mobile), use [Directory Services](/Directory_Services) instead

## Source register

| Topic | Status | Source |
| ----- | ------ | ------ |
| IP-to-group mapping, first-match order | **Confirmed** | This page |
| PAM FALSE for transparent IP policy | **Confirmed** | Troubleshooting table |

## Next steps

- **Add user authentication:** Combine Network Signature with [Directory Services](/Directory_Services) or [BASIC](/BASIC) for user-aware policies
- **Refine access rules:** Use [Access Restriction](/Access_Restriction) to define what each user-group can access
- **Enable SSL Inspection:** Configure [SSL Inspection](/SSL_Inspection) to inspect HTTPS traffic from IP-based groups
